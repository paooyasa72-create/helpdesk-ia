...
async function enviarRespuestaFAQ(ticket, textoRespuesta) {
  ...
  const asunto = `Re: ${ticket.titulo} — Posible solución generada por IA`;

  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 500px;">
      <h2 style="color:#3fb27f;">🤖 Posible solución generada por IA</h2>
      <p>Hola, el agente de inteligencia artificial del Help Desk analizó tu ticket y encontró una posible solución:</p>
      <p><b>Tu consulta:</b> ${ticket.titulo}</p>
      <div style="background:#f4f4f4; padding:14px; border-radius:8px; margin:14px 0; white-space:pre-line;">
        <b>Posible solución:</b><br>${textoRespuesta}
      </div>
      <p>Este ticket se cerró automáticamente porque fue identificado como una pregunta frecuente. Si tu problema no se resolvió con esta solución, puedes crear un nuevo ticket y un técnico lo revisará.</p>
      <p style="color:#888; font-size:12px;">Respuesta generada automáticamente por el agente de IA del Help Desk Inteligente.</p>
    </div>
  `;
